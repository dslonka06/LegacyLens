class RecommendationAnalysisEngine {
  ...
  analyzeFile(session) {
    ...
    if (session.aiAnalysis?.risks?.length) {
      session.aiAnalysis.risks.forEach((r, i) => {
    ...
    if (session.aiAnalysis?.modernizations?.length) {
      session.aiAnalysis.modernizations.forEach((m, i) => {
        recs.push({
          id: `modern-${i}`,
          title: m.title,
          priorityScore: 30,
  ...
  buildAnalysis(recs, primaryFile) {
    const sorted = [...recs].sort((a, b) => b.priorityScore - a.priorityScore);
    const ranked = sorted.map((r, i) => ({ ...r, priorityRank: i + 1 }));
  ...
  deriveModernizationReadiness(recs) {
  ...
  buildOverview(recs, primaryFile, criticalCount, highCount) {
    if (recs.length === 0) {
  ...
  buildModernizationContext(readiness, recs) {
  ...
  buildModernizationAssessment(readiness, recs, primaryFile) {
    const modItems = recs.filter(r => r.category === 'modernization');
  ...
  firstFileName(sourceFiles) {
    return sourceFiles[0]?.path.split('/').pop() ?? sourceFiles[0]?.path ?? 'Unknown';
  }

  severityToScore(sev) {
    return ({ critical: 95, high: 75, medium: 50, low: 30, info: 20 })[sev] ?? 40;
  }

  severityToPriority(sev) {
    return ({ critical: 'critical', high: 'high', medium: 'medium', low: 'low', info: 'low' })[sev] ?? 'medium';
  }

  severityToRisk(sev) {
    return ({ critical: 'high', high: 'high', medium: 'medium', low: 'low', info: 'low' })[sev] ?? 'medium';
  }

  inferCategory(title, desc, fallback) {
  ...
  deriveExpectedImpact(title, sev) {
...
module.exports = { RecommendationAnalysisEngine };
