...
class DataFlowDiscoveryEngine {
  ...
  discoverWorkflows(knowledge, structure) {
  ...
  discoverStructuralFlows(typed, graph, structure) {
    const flows = [];
    ...
    for (const [feature, entries] of byFeature) {
      ...
      for (const targetId of directTargets.slice(0, 8)) {
        ...
        const children = (outboundEdges.get(target.id) ?? [])
          .map(id => typed.get(id))
          .filter((n) => !!n && n.type !== 'unknown' && !reachable.some(r => r.id === n.id))
  ...
  extractBehaviorInsights(knowledge) {
    const graph = knowledge.dependencyGraph;
    if (!graph) {
  ...
  traceForward(nodeId, typed, graph, visited, maxDepth) {
...
module.exports = { DataFlowDiscoveryEngine };
