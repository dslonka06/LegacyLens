...
class RepositoryInsightsEngine {
  ...
  insightsForNode(nodeId, knowledge) {
  ...
  analyze(knowledge) {
    ...
    const rankings = this.dependencyExplorer.rankByConnectivity(graph, graph.nodes.length);
    ...
    const hubs = this.dependencyExplorer.dependencyHubs(graph, HUB_DEGREE_MULTIPLIER);
    if (hubs.length > 0) {
    ...
    // ── Summary stat ─────────────────────────────────────────────────────
    const avg = this.dependencyExplorer.averageConnectivity(graph);
    insights.push({
...
module.exports = { RepositoryInsightsEngine };
