...
class LearningPathAnalysisEngine {
  ...
  buildFileWelcome(session, u, systemType) {
  ...
  buildFileReadingOrder(session, u) {
    const items = [];
    items.push({
      rank: 1,
  ...
  buildKnowledgeWelcome(knowledge, u, systemType, systemName) {
  ...
  buildKnowledgeFocusFirst(knowledge, u) {
  ...
  buildKnowledgeRoadmap(knowledge, u, scope) {
    const steps = [];
  ...
  buildKnowledgeAreas(knowledge, u) {
    const areas = [];
    const graph = knowledge.dependencyGraph;
    ...
    for (const node of graph.nodes) {
      ...
      if (name) {
        ...
        existing.push({ name, score: inbound.get(node.id) ?? 0 });
  ...
  buildKnowledgeReadingOrder(knowledge, u) {
    const items = [];
    const graph = knowledge.dependencyGraph;
  ...
  buildKnowledgeIgnoreList(knowledge, u) {
    const ignore = [];
    const graph = knowledge.dependencyGraph;
  ...
  extractConceptsFromText(text, u) {
    const found = [];
    const seen = new Set();
  ...
  inferSystemTypeFromFile(session) {
  ...
  inferSystemTypeFromKnowledge(knowledge, u) {
    const allText = [
...
module.exports = { LearningPathAnalysisEngine };
