...
class SecurityAnalysisEngine {
  ...
  detectFileFindings(filePath, content, findings) {
  ...
  buildAnalysis(findings, relevant, hotspots, now) {
  ...
  deriveAssessment(findings, risk, relevant) {
  ...
  deriveExecutiveSummary(findings, risk, maturity, relevant) {
    const count = findings.length;
    const critical = findings.filter(f => f.severity === 'critical').length;
    const high = findings.filter(f => f.severity === 'high').length;
    const cats = [...new Set(findings.map(f => f.category))];
  ...
  deriveRiskContext(findings, risk) {
    const count = findings.length;
  ...
  deriveMaturityContext(maturity, findings) {
    const cats = [...new Set(findings.map(f => f.category))];
  ...
  getDirectDependents(nodeId, graph, limit) {
