...
const { RepositoryInsightsEngine } = require('./repository-insights.engine');
const { DataFlowDiscoveryEngine } = require('./data-flow-discovery.engine');
const { WorkflowExplorerEngine } = require('./workflow-explorer.engine');

class RepositorySummaryEngine {

  constructor() {
    this.dependencyExplorer = new DependencyExplorerEngine();
    this.insightsService = new RepositoryInsightsEngine();
    this.dataFlowDiscovery = new DataFlowDiscoveryEngine();
    this.workflowExplorer = new WorkflowExplorerEngine();
  }
  ...
  build(workspaceContext, knowledge, session) {
    ...
    if (knowledge?.dependencyGraph) {
      ...
      summary.dependencyStats = {
        ...
        averageConnectivity: this.dependencyExplorer.averageConnectivity(graph),
      };
    ...
    // ── Repository Insights ───────────────────────────────────────────────
    if (knowledge) {
      const raw = this.insightsService.analyze(knowledge);
  ...
  buildExecutiveSummary(ctx, knowledge, session, profile) {
    const parts = [];
  ...
  buildRepositoryOverview(ctx, profile, knowledge) {
    const parts = [];
  ...
  buildArchitectureSummary(knowledge, profile) {
  ...
  buildDataFlowSummary(session, knowledge) {
    const flow = session?.analysis?.dataFlow;
    if (flow) return flow;
    if (knowledge?.dependencyGraph) {
      const g = knowledge.dependencyGraph;
      const avg = this.dependencyExplorer.averageConnectivity(g);
  ...
  buildDependencySummary(knowledge) {
    const graph = knowledge.dependencyGraph;
    const top = this.dependencyExplorer.rankByConnectivity(graph, 3);
    const hubs = this.dependencyExplorer.dependencyHubs(graph);
    const orphans = this.dependencyExplorer.orphanedFiles(graph);
  ...
  buildRisks(session, knowledge) {
    ...
    if (knowledge) {
      const insightRisks = this.insightsService.analyze(knowledge)
  ...
  buildKeyFiles(session, knowledge, profile) {
    const keys = [];
    const seen = new Set();
    ...
    if (knowledge?.dependencyGraph) {
      const top = this.dependencyExplorer.rankByConnectivity(knowledge.dependencyGraph, 10);
      for (const r of top) {
  ...
  buildOnboardingNotes(ctx, knowledge, session) {
    const parts = [];
  ...
  countFolders(folder) {
    return folder.children.reduce((n, c) => n + 1 + this.countFolders(c), 0);
  }
}

module.exports = { RepositorySummaryEngine };
