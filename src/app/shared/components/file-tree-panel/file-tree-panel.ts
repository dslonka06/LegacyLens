...
export class FileTreePanel implements OnChanges {
  ...
  private expandAncestors(node: FolderNode, target: string): boolean {
    ...
    const inChildren = node.children.some(c => target.startsWith(c.path) || this.expandAncestors(c, target));
  ...
  private rebuildRows(): void {
    if (!this.folderTree) { this.rows = []; return; }
    const rows: TreeRow[] = [];
  ...
  private walkFolder(node: FolderNode, depth: number, rows: TreeRow[]): void {
    ...
    if (isRoot || expanded) {
      ...
      for (const file of node.files) {
        rows.push({ kind: 'file', node: file, folderPath: node.path, depth: depth + (isRoot ? 0 : 1), visible: true });
      }
    }
  }
}
