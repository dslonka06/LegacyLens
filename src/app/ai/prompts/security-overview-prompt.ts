...
export class SecurityOverviewPromptBuilder {
  build(ctx: SecurityOverviewContext): string {
    ...
    parts.push(
      `Write in plain prose. No bullet lists. No headers. Do not invent details not present in the data. Do not recommend fixes — describe the current state only.`,
    );
    parts.push(``);
    ...
    if (scope === 'file') {
      if (totalFindings === 0) {
        parts.push(
          `Output: one sentence only. State that no significant security concerns were detected in this file.`,
        );
      } else {
        parts.push(
          `Output: 1 short paragraph, 40–80 words. Characterise the security posture of this single file. Name the most significant finding if one exists. Do not pad with generic advice.`,
        );
      }
    } else if (scope === 'folder') {
      parts.push(
        `Output: 2 short paragraphs, 80–150 words total. Cover: (1) overall posture of this folder, (2) the most significant concern if any findings exist. Stop there.`,
      );
    } else {
      parts.push(
        `Output: 3–4 paragraphs, 150–250 words. Cover: (1) overall posture and what drives the risk level, (2) most significant concerns, (3) confidence in the findings, (4) readiness for production use.`,
      );
    }
    ...
    if (security.relevantComponents.length > 0 && scope !== 'file') {
      const names = security.relevantComponents
        .slice(0, 5)
        .map((c) => c.name)
        .join(', ');
    ...
    if (totalFindings > 0) {
      ...
      for (const f of top) {
        parts.push(
          `- [${f.severity}] ${f.title}${scope !== 'file' ? ` (${f.fileName})` : ''}: ${f.issueDescription}`,
        );
      }
    }
    ...
    parts.push(
      ``,
      `Do not use section headings. Synthesise the data above into a professional narrative matching the output format above.`,
    );
