...
import { RepositoryKnowledge, DependencyGraph } from '@app/knowledge/models/knowledge.model';
...
export class SystemUnderstandingService {
  ...
  private fileHealth(session: AnalysisSession): SystemHealthSummary {
    ...
    const modernizationReadiness: HealthLevel = a.modernizationSuggestions.length >= 3 ? 'Low'
  ...
  private knowledgeHealth(
    ...
    const modernizationReadiness: HealthLevel = modernizationCount >= 5 ? 'Low' : modernizationCount >= 2 ? 'Medium' : 'High';
